...
Post nextPost(Platform platform) {
    ...
    if (platform->lastViewedPost->nxtPost == NULL) {
        return NULL;
    }

    platform->lastViewedPost = platform->lastViewedPost->nxtPost;
...
Post previousPost(Platform platform) {
    ...
    if (platform->lastViewedPost->prevPost == NULL) {
        fprintf(stderr, "No previous post available.\n");
        return NULL;
    }

    Post current = platform->lastViewedPost->prevPost;
